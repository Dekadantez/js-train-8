...
function findElementIndexes(arr, element) {
  ...
  let elem1 = arr.indexOf(element);
  let elem2 = arr.lastIndexOf(element);

  let elem3 = [elem1, elem2];
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  let array = arr.copyWithin(target, start, end);
  return array;
...
function customUnshift(arr, ...elements) {
  ...
  // Додаємо елементи на початок масиву за допомогою методу `unshift` та циклу for, початкове значення лічильника
  //на 1 менше ніж довжина масива,
...
function customIncludes(arr, element) {
  ...
  let res = arr.filter((item) => item === element).length;
  console.log(res);
  return result;
  // Перевіряємо, чи вхідний параметр є масивом
